...
function AASNodeMapping(node, configJson, nodeName, submodelIdShort) {
    ...
    switch (types['elemModelType']) {
        case 'Property':
            configJson.iota.types[`${submodelIdShort}`].lazy.push({
                name: nodeName + '_' + types['elemName'],
                type: types['elemType']
            });

            configJson.iota.contextSubscriptions[configJson.iota.contextSubscriptions.length - 1]['mappings'].push({
                ocb_id: nodeName + '_' + types['elemName'],
                submodel_element_short_id: types['elemName']
            });
            break;
        case 'Operation':
            configJson.iota.types[`${submodelIdShort}`].commands.push({
                name: nodeName + '_' + types['elemName'],
                type: types['elemType']
            });
            break;
