...
function AASNodeMapping(types, configJson, nodeName, submodelIdShort, element) {
    const newType = {
    ...
    // Extract value from element if it's an active attribute
    if (types['isAttributeActive'] && element) {
        const value = extractValueFromElement(element);
        if (value !== null && value !== undefined) {
            newSubscription.initialValue = value;
        }
    }
...
function extractValueFromElement(element) {
    ...
    const modelType = typeof element.modelType === 'object' ? element.modelType.name : element.modelType;
