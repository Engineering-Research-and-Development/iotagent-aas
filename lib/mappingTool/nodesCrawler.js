...
async function nodesCrawler(submodel, configJson, shellId = undefined) {
    ...
    if (shellId === undefined) {
        ...
        rootElement = shellId + config.getConfig().attrSeparatorCharacter + types['elemName'];
    }
    ...
    if (collectionRecognition.isSumodelElementCollection(types['elemModelType']) || collectionRecognition.isSumodelElementList(types['elemModelType'])) {
        if (element.hasOwnProperty('value')) {
            for (let elem of element['value']) {
                recursiveCrawling(elem, configJson, nodeName + config.getConfig().attrSeparatorCharacter + types['elemName'], submodelIdShort);
            }
        }
        ...
        if (element.hasOwnProperty('statements')) {
            for (let statement of element['statements']) {
                recursiveCrawling(statement, configJson, nodeName + config.getConfig().attrSeparatorCharacter + types['elemName'], submodelIdShort);
            }
        }
    } else {
