...
async function mappingTool(configJS) {
    ...
    async function mappingToolRun() {
        try {
            ...
            await client.getSubmodels(configJS.aas.endpoint, configJS.aas.api_version, aasInformation);
            //await client.getSubmodelsFromLocalData(localFIle, aasInformation); use local files

            if (aasInformation.length === 0) {
            ...
            if (configJS.aas.api_version === 'v1') {
                for (let submodel of aasInformation) {
                    await nodesCrawler.nodesCrawler(submodel, configJson);
                }
            } else {
                for (let AASShell of aasInformation) {
                    let shellId = AASShell['idShort'];
                    for (let submodel of AASShell['submodels']) {
                        await nodesCrawler.nodesCrawler(submodel, configJson, shellId);
                    }
                }
            }
