    ...
    if (device && device.apikey) {
        ...
        iotAgentLib.findConfiguration(service, subservice, type, function (error, group) {
            if (group) {
                ...
                config.getLogger().error(context, 'Could not find any APIKey information for device in service %s subservice %s and type %s', service, subservice, type);
...
function manageConfiguration(apiKey, deviceId, device, objMessage, sendFunction, callback) {
    ...
    if (objMessage.type === 'configuration') {
        async.waterfall([apply(iotAgentLib.query, device.name, device.type, '', objMessage.fields, device), apply(sendFunction, apiKey, {}, deviceId)], handleSendConfigurationError);
    } else if (objMessage.type === 'subscription') {
        iotAgentLib.subscribe(device, objMessage.fields, objMessage.fields, function (error) {
            if (error) {
                config.getLogger().error(context, 'CONFIG-002: There was an error subscribing device %s to attributes %j', device.name, objMessage.fields);
            } else {
    ...
    if (apiKey === config.getConfig().defaultKey) {
        ...
        async.waterfall(
            [
                ...
                apply(iotAgentLib.mergeDeviceWithConfiguration, ['lazy', 'active', 'staticAttributes', 'commands', 'subscriptions'], [null, null, [], [], [], [], []])
            ],
            callback
        );
    }
