...
function processEnvironmentVariables() {
    ...
    const aasVariables = ['IOTA_AAS_ENDPOINT', 'IOTA_AAS_API_VERSION'];

    const protectedVariables = ['IOTA_MQTT_KEY', 'IOTA_MQTT_USERNAME', 'IOTA_MQTT_PASSWORD', 'IOTA_HTTP_KEY'];

    const mappingToolVariables = ['IOTA_AAS_MT_AGENT_ID', 'IOTA_AAS_MT_ENTITY_ID', 'IOTA_AAS_MT_ENTITY_TYPE', 'IOTA_AAS_MT_STORE_OUTPUT', 'IOTA_AAS_MT_ATTR_SEPARATOR_CHARACTER'];

    commonVariables.forEach((key) => {
        let value = process.env[key];
        if (value) {
            logger.info('Setting AAS IoTAgent environment variable %s to environment value: %s', key, value);
        }
    });
    ...
    if (process.env.IOTA_AAS_API_VERSION) {
        config.aas.api_version = process.env.IOTA_AAS_API_VERSION;
    }
    ...
    if (process.env.IOTA_AAS_MT_ATTR_SEPARATOR_CHARACTER) {
        config.mappingTool.attrSeparatorCharacter = process.env.IOTA_AAS_MT_ATTR_SEPARATOR_CHARACTER;
    }
}
