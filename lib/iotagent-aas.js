...
function createNewType(device, conf) {
    conf.iota.types[`${device.type}`] = {
        active: [],
        lazy: device.active.concat(device.lazy),
...
function appendTypeAttributesAndCommands(device, conf) {
    let allDeviceAttributes = device.active.concat(device.lazy);
    let allConfTypeAttributes = conf.iota.types[`${device.type}`].lazy;

    for (let deviceAttribute of allDeviceAttributes) {
        if (allConfTypeAttributes.indexOf(deviceAttribute) === -1) {
            conf.iota.types[`${device.type}`].lazy.push(deviceAttribute);
        }
    }

    let allDeviceCommands = device.commands;
    let allConfTypeCommands = conf.iota.types[`${device.type}`].commands;

    for (let deviceCommand of allDeviceCommands) {
        if (allConfTypeCommands.indexOf(deviceCommand) === -1) {
