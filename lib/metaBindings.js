...
function getInitialValuesFromMappings(device, conf) {
    const initialValues = {};

    // Find context subscription for this device
    const foundContext = conf.iota.contextSubscriptions.find((ctx) => ctx.id === device.name);

    if (!foundContext || !foundContext.mappings) {
        config.getLogger().debug(context, 'No context or mappings found for device %s', device.name);
        return initialValues;
    }

    // Extract initial values from mappings
    for (const activeAttribute of device.active) {
        ...
        if (mapping && mapping.initialValue !== undefined) {
            initialValues[activeAttribute.name] = mapping.initialValue;
            config.getLogger().debug(context, 'Found initial value for %s: %j', activeAttribute.name, mapping.initialValue);
        }
    }
...
function initActiveAttributes(device) {
    return new Promise((resolve, reject) => {
        ...
        // Get initial values from mappings
        const initialValues = getInitialValuesFromMappings(device, conf);
        ...
        if (ngsiVersion === 'ld') {
            ...
            for (const activeAttribute of device.active) {
                const value = initialValues[activeAttribute.name] !== undefined ? initialValues[activeAttribute.name] : '';
            ...
            for (const activeAttribute of device.active) {
                ...
                body[`${activeAttribute.name}`] = {
                    value,
                    type: activeAttribute.type || 'Text'
                };
            }
