...
async function initProvisioning() {
    try {
        if (config.getConfig().configurationType === 'auto' || config.getConfig().configurationType === 'static') {
            ...
            for (let contextSubscription of config.getConfig().iota.contextSubscriptions) {
                await sendEmptyData(contextSubscription);
            }
        }
    } catch (err) {
        /* istanbul ignore next */
...
function sendEmptyData(contextSubscription, device = undefined) {
    return new Promise((resolve, reject) => {
        ...
        let servicepath;
        if (device === undefined) {
            service = conf.iota.service;
            servicepath = conf.iota.subservice;
        } else {
            service = device.service;
            servicepath = device.subservice;
        }

        const body = {
            id: contextSubscription.id,
            type: contextSubscription.type
        };

        let activeAttributes = conf.iota.types[`${contextSubscription.type}`].active;

        for (let attribute of contextSubscription.mappings) {
            let attributeType;
            for (let activeAttribute of activeAttributes) {
                if (attribute.ocb_id === activeAttribute.name) {
                    attributeType = activeAttribute.type;
                    body[`${attribute.ocb_id}`] = {
                        value: '',
                        type: attributeType
                    };
                    break;
        ...
        fetch(`http://${conf.iota.contextBroker.host}:${conf.iota.contextBroker.port}/v2/entities`, {
            method: 'POST',
            headers: {
...
exports.sendEmptyData = sendEmptyData;
