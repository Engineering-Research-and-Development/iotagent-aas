...
function parseMessage(message) {
    ...
    config.getLogger().debug(context, 'parserMessage array: %s', JSON.stringify(messageArray));
    return messageArray;
}
...
function sendConfigurationToDevice(device, apiKey, group, deviceId, results, callback) {
    iotAgentLib.getConfigurationSilently(config.getConfig().iota.defaultResource || '', apiKey, function (error, foundGroup) {
        ...
        transportSelector.applyFunctionFromBinding([apiKey, group, deviceId, results], 'sendConfigurationToDevice', device.transport || group.transport || config.getConfig().defaultTransport, callback);
    });
}
    ...
    for (let i = 0; i < objMessage.length; i++) {
        iotaUtils.manageConfiguration(apiKey, deviceId, device, objMessage[i], async.apply(sendConfigurationToDevice, device), function (error) {
            if (error) {
                ...
                config.getLogger().debug(ctxt, 'Configuration request finished for APIKey %s and Device %s', apiKey, deviceId);
            }
...
function multipleMeasures(apiKey, deviceId, device, messageObj) {
    ...
    for (let j = 0; j < messageObj.length; j++) {
        ...
        for (const k in attributesArray) {
            config.getLogger().debug(context, 'Processing multiple measures for device %s with apiKey %s values %j', deviceId, apiKey, attributesArray[k]);
            iotAgentLib.update(device.name, device.type, '', attributesArray[k], device, function (error) {
                if (error) {
                    ...
                    config.getLogger().info(ctxt, 'Multiple measures for device %s with apiKey %s successfully updated', deviceId, apiKey);
                }
                finishSouthBoundTransaction(null);
            });
        }
...
function messageHandler(topic, message) {
    ...
    if (topic.startsWith('/sm-repository/sm-repo/submodels/')) {
        if (topic.startsWith('/sm-repository/sm-repo/submodels/')) {
            if (topicInformation[4] === 'created') {
                commandHandler.handleSubmodelCreated(parsedMessage);
            } else if (topicInformation[4] === 'deleted') {
                console.log('Handling submodel deleted:', parsedMessage);
                commandHandler.handleSubmodelDeleted(parsedMessage);
            }
        }
    } else {
        iotAgentLib.alarms.release(constants.MQTTB_ALARM);
...
function navigateAASTree(data, elementRequested, pathToTheElement) {
    if (elementRequested === pathToTheElement[0]) {
        return extractAASData(data);
    } else {
        remainingPath = pathToTheElement.slice(1);
        let nextElement;
        let modelType = getModelType(data.modelType);
        switch (modelType) {
            case 'Entity':
                for (let statement of data['statements']) {
                    if (statement['idShort'] === remainingPath[0]) {
                        nextElement = statement;
                        break;
                    }
                }
                return navigateAASTree(nextElement, elementRequested, remainingPath);

            case 'SubmodelElementCollection':
                for (let elem of data['value']) {
                    if (elem['idShort'] === remainingPath[0]) {
                        nextElement = elem;
                        break;
                    }
                }
            ...
            default:
                config.getLogger().error(context, `QUERY-002: New ModelType ${modelType} detected, update navigateAASTree function`);
                break;
        }
    }
}

function extractAASData(data) {
    ...
    switch (modelType) {
        ...
        case 'MultiLanguageProperty':
            return data.value;
        default:
            config.getLogger().error(context, `QUERY-002: New ModelType ${modelType} detected, update extractAASData function`);
            break;
    }
}
...
exports.navigateAASTree = navigateAASTree;
