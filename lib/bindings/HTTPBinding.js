...
async function executeQuery(apiKey, device, attribute, callback) {
    ...
    for (const context of config.getConfig().iota.contextSubscriptions) {
        for (const mapping of context.mappings) {
            if (mapping.ocb_id === attribute) {
                ...
                foundMapping.submodel_short_id = context.submodel_short_id;
                break;
            }
        }
    }
    let value = null;
    try {
        value = await readValueFromAAS(device, foundMapping.submodel_short_id, foundMapping.submodel_element_short_id);
    } catch (e) {
...
async function readValueFromAAS(device, submodelIdShort, submodelElementIdShort) {
    const url = `${device.endpoint}/submodels/${submodelIdShort}/submodel/submodelElements/${submodelElementIdShort}`;
        ...
        inputArguments: [
            {
                ...
                value: {
                    ...
                    valueType: 'string',
                    value: serializedPayload[cmdName]
                }
            },
            {
                modelType: {
