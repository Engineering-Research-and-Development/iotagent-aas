...
async function executeQuery(apiKey, device, attribute, callback) {
    ...
    let value = null;
    try {
        value = await readValueFromAAS(device, foundMapping.submodel_short_id, foundMapping.submodel_element_short_id, foundMapping.ocb_id);
    } catch (e) {
...
async function readValueFromAAS(device, submodelIdShort, submodelElementIdShort, aasElementPath) {
    let pathToTheElement = aasElementPath.split('_');
    pathToTheElement = pathToTheElement.slice(1);
    let url;

    if (config.getConfig().aas.api_version === 'v1') {
        url = `${device.endpoint}/submodels/${submodelIdShort}/submodel/submodelElements/${pathToTheElement[0]}`;
    } else {
        url = `${device.endpoint}/submodels/${btoa(device.id)}/submodel-elements/${pathToTheElement[0]}`;
    }

    const res = await axios.get(url);
    if (res && res.status === 200) return commonBindings.navigateAASTree(res.data, submodelElementIdShort, pathToTheElement);
