...
function generateTopics(callback) {
    ...
    topics.push(constants.MQTT_SHARE_SUBSCRIPTION_GROUP + '/+/+/' + constants.MEASURES_SUFIX);
    topics.push(constants.MQTT_SHARE_SUBSCRIPTION_GROUP + '/' + constants.MQTT_TOPIC_PROTOCOL + '/+/+/' + constants.MEASURES_SUFIX);
    topics.push(constants.MQTT_SHARE_SUBSCRIPTION_GROUP + '/+/+/' + constants.CONFIGURATION_SUFIX + '/' + constants.CONFIGURATION_COMMAND_SUFIX);
    topics.push(constants.MQTT_SHARE_SUBSCRIPTION_GROUP + '/' + constants.MQTT_TOPIC_PROTOCOL + '/+/+/' + constants.CONFIGURATION_SUFIX + '/' + constants.CONFIGURATION_COMMAND_SUFIX);
    topics.push(constants.MQTT_SHARE_SUBSCRIPTION_GROUP + '/+/+/' + constants.CONFIGURATION_COMMAND_UPDATE);
    topics.push(constants.MQTT_SHARE_SUBSCRIPTION_GROUP + '/' + constants.MQTT_TOPIC_PROTOCOL + '/+/+/' + constants.CONFIGURATION_COMMAND_UPDATE);
    ...
    // submodels topics
    topics.push('sm-repository/sm-repo/submodels/#');
...
function sendConfigurationToDevice(apiKey, group, deviceId, results, callback) {
    ...
    const commandTopic = leadingSlash + apiKey + '/' + deviceId + '/' + constants.CONFIGURATION_SUFIX + '/' + constants.CONFIGURATION_VALUES_SUFIX;
    //prettier-ignore
...
function unsubscribeAll(callback) {
    ...
    try {
        async.waterfall([generateTopics, unsubscribeFromTopics], function (err) {
            // Ignore errors during unsubscription on shutdown
            if (err) {
                config.getLogger().debug(context, 'Error during unsubscribeAll (ignoring): %s', err.message);
            }
            callback();
        });
    } catch (err) {
        config.getLogger().debug(context, 'Exception during unsubscribeAll (ignoring): %s', err.message);
        callback();
    }
}
...
function start(callback) {
    ...
    const rejectUnauthorized = typeof mqttConfig.rejectUnauthorized === 'boolean' ? mqttConfig.rejectUnauthorized : true;
    ...
    const options = {
        ...
        clientId: mqttConfig.clientId ? mqttConfig.clientId + rndSuffix : 'iotaaas' + rndSuffix,
    ...
    config.getLogger().info(context, 'Starting MQTT binding with options %j retries %s retryTIme %s', options, retries, retryTime);
    ...
    function createConnection(callback) {
        ...
        options.clientId = mqttConfig.clientId ? mqttConfig.clientId + rndSuffix : 'iotaaas' + rndSuffix;
...
function executeCommand(apiKey, group, device, cmdName, serializedPayload, contentType, callback) {
    ...
    options.qos = commands[cmdName].mqtt && commands[cmdName].mqtt.qos ? parseInt(commands[cmdName].mqtt.qos) : config.getConfig().mqtt.qos ? parseInt(config.getConfig().mqtt.qos) : 0;
    options.retain = commands[cmdName].mqtt && commands[cmdName].mqtt.retain ? commands[cmdName].mqtt.retain : config.getConfig().mqtt.retain ? config.getConfig().mqtt.retain : false;

    const commandTopic = '/' + apiKey + '/' + device.id + '/cmd';
    config.getLogger().debug(context, 'Sending command execution to %s with payload %s and with mqtt options %j', commandTopic, serializedPayload, options);
    if (mqttClient.connected) {
        mqttClient.publish(commandTopic, serializedPayload, options, (error) => {
            if (error) {
                config.getLogger().error(context, 'Error %j in Cmd: %j sent to the device %s with mqtt options %j', error, serializedPayload, commandTopic, options);
            }
        });
        config.getLogger().info(context, 'Cmd: %j was sent to the device %s with mqtt options %j', serializedPayload, commandTopic, options);
    } else {
